...
import src.validaciones_json.constantes_json as json_const
...
def verificacion_script_argumento_json(argumento_script_json):
    ...
    if not FormatUtils.verificar_keys_json(json_argumento_formateado):
        ...
        print(
            'La imagen/archivo por cargar dentro de la plataforma Claro Drive no fue localizado dentro del server, '
...
def verificacion_script_argumentos():
    """
    Verifica que dentro de la ejecucion del script se haya establecido el parametro principal en formato JSON. De
    ser asi la funcion retornara True, en caso contrario retorna False
...
def verificacion_archivo_config(archivo_config: configparser.ConfigParser):
    ...
    bool_url_login = archivo_config.has_option('Dropbox_config', 'url_login')
    ...
    if not bool_ruta:
        ...
        validacion_total = False
    elif not bool_url_login:
        print('Favor de establecer la opcion/configuracion url login dentro del archivo config.ini')
        validacion_total = False
...
def verificacion_estatus_final(json_evaluacion):
    val_paso_1 = True if json_evaluacion["steps"][0]["status"] == json_const.SUCCESS else False
    val_paso_2 = True if json_evaluacion["steps"][1]["status"] == json_const.SUCCESS else False
    val_paso_3 = True if json_evaluacion["steps"][2]["status"] == json_const.SUCCESS else False
    val_paso_4 = True if json_evaluacion["steps"][3]["status"] == json_const.SUCCESS else False
    val_paso_5 = True if json_evaluacion["steps"][4]["status"] == json_const.SUCCESS else False
    val_paso_6 = True if json_evaluacion["steps"][5]["status"] == json_const.SUCCESS else False

    eval_final = val_paso_1 and val_paso_2 and val_paso_3 and val_paso_4 and val_paso_5 and val_paso_6

    return json_const.SUCCESS if eval_final else json_const.FAILED
